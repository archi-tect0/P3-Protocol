...
const BASE_TO_POLYGON_BRIDGE = process.env.BASE_POLYGON_BRIDGE_ADDRESS || '0x0000000000000000000000000000000000000000';
...
export class BaseToPolygonAdapter implements BridgeAdapter {
  ...
  constructor() {
    ...
    this.contract = new Contract(BASE_TO_POLYGON_BRIDGE, BRIDGE_ABI, this.provider);
  }
  ...
  async relayReceipt(docHash: string, receiptData: any): Promise<{ txHash: string }> {
    try {
      ...
      console.error('Base->Polygon relay error:', error);
      throw new Error(`Failed to relay to Polygon: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
