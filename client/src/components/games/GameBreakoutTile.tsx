...
export default function GameBreakoutTile() {
  const gameId = `breakout-${Date.now()}`;

  async function handlePlay() {
    try {
      await P3.proofs.publish("game_start", { gameId, game: "breakout", score: 0, ts: Date.now() });
    } catch (e) {
      console.warn("Anchor failed:", e);
    }
    window.open('/games/breakout/', '_blank');
  }

  return (
    <Card className="glass-card overflow-hidden" data-testid="tile-game-breakout">
      <CardContent className="p-0">
        ...
        <div className="p-4 text-center">
          ...
          <Button onClick={handlePlay} data-testid="button-play-breakout">Play</Button>
        </div>
      </CardContent>
    </Card>
  );
