...
export default function ChatMode() {
  ...
  const { wallet, setMode, setPulseView, setActiveTab, setTypingIntensity, setVoiceActive } = useAtlasStore();
  ...
  useEffect(() => {
    if (wallet && !sessionStarted) {
      startAtlasSession();
    }
  }, [wallet, sessionStarted]);
  ...
  async function fetchConnectedApps() {
    if (!wallet) return;
    try {
      const response = await fetch(`/api/atlas/mesh/connected?wallet=${wallet}`);
  ...
  async function startAtlasSession() {
    ...
    try {
      ...
      if (authToken) {
        try {
          ...
          if (parts.length === 3) {
            ...
            if (expiry <= Date.now()) {
              console.log('[ChatMode] JWT expired, need re-authentication');
          ...
          console.log('[ChatMode] JWT decode error:', e);
          needsAuth = true;
          authToken = null;
        }
      }
      ...
      if (needsAuth && wallet) {
        ...
        if (bridgeSession?.connected && bridgeSession.address?.toLowerCase() === wallet.toLowerCase()) {
          console.log('[ChatMode] Bridge session exists, skipping re-auth');
          try {
            const statusRes = await fetch(`/api/atlas/session/bridge/status?wallet=${wallet}`);
            if (statusRes.ok) {
              ...
              if (statusData.token) {
                ...
                needsAuth = false;
                console.log('[ChatMode] Restored session from bridge status');
              }
            }
          } catch (e) {
            console.log('[ChatMode] Bridge status check failed:', e);
          }
        }
        
        if (needsAuth) {
          console.log('[ChatMode] No valid JWT or bridge session, triggering wallet authentication');
          const signMessage = async (msg: string) => signWithBridge(msg);
          const authenticated = await authenticateWallet(wallet, signMessage);
          
          if (!authenticated) {
            console.log('[ChatMode] Authentication failed or cancelled');
            return;
          }
  ...
  async function sendMessage(text?: string) {
    ...
    if (!messageText || loading || !wallet) return;
  ...
  }, [sessionStarted, wallet, loading, input, toast]);
    ...
    try {
      const response = await fetch('/api/atlas/call', {
        ...
        body: JSON.stringify({
          ...
          args: endpoint.args,
          session: { wallet, roles: ['user'], grants: [] },
        }),
      });
  ...
  if (!wallet) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 p-6" data-testid="chat-no-wallet">
        <Bot className="w-12 h-12 text-white/20" />
        <p className="text-white/60 text-center">Connect your wallet to chat with Atlas</p>
      </div>
    );
  }
  ...
  return (
    <div 
      ...
      data-testid="chat-mode-container"
    >
      {connectedApps.length > 0 && (
        <div className="px-4 pt-3 pb-2 border-b border-white/5" data-testid="connected-apps-bar">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 pt-3 space-y-3" data-testid="messages-container">
      ...
      <div className="flex-shrink-0 p-4 border-t border-white/5" data-testid="input-bar">
        <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() => {
                import('@/lib/diag').then(({ diag }) => {
                  diag('ChatMode', 'Mic button clicked', { isRecording, isProcessing, loading, isVoiceSupported });
                });
                if (isRecording) {
                  stopCapture();
                } else {
          ...
          <Input
            ...
            placeholder={isRecording ? "Listening..." : "Ask Atlas anything..."}
