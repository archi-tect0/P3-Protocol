...
import { MarketplaceContextSheet, type AssetType } from '@/components/MarketplaceContextSheet';
...
function TrackCard({ 
  ...
  return (
    <Card
      ...
      className={`group relative overflow-hidden bg-card/80 dark:bg-slate-900/80 backdrop-blur-xl border-border/5 hover:border-cyan-500/30 transition-all duration-300 cursor-pointer touch-manipulation ${isLongPressing ? 'scale-[0.98] ring-2 ring-cyan-500/50' : ''}`}
      {...handlers}
    >
      <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/5 to-purple-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <CardContent className="relative p-4">
        <div className="aspect-square rounded-lg bg-gradient-to-br from-cyan-500/20 to-purple-500/20 mb-4 flex items-center justify-center overflow-hidden relative group">
          {track.coverUrl ? (
            <img
              src={track.coverUrl}
              alt={track.title}
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            <div className="w-12 h-12 rounded-full bg-cyan-500 flex items-center justify-center transform scale-90 group-hover:scale-100 transition-transform">
              <Play className="w-6 h-6 text-white fill-white ml-1" />
            </div>
          </div>
        </div>
        <h3 className="font-semibold text-white mb-1 line-clamp-1 group-hover:text-cyan-300 transition-colors">
          {track.title}
        </h3>
        <p className="text-sm text-slate-400 mb-2 truncate flex items-center gap-1">
          <User className="w-3 h-3" />
          {track.authorWallet.slice(0, 6)}...{track.authorWallet.slice(-4)}
        </p>
...
function AlbumCard({ 
  ...
  return (
    ...
    >
      ...
      <CardContent className="relative p-4">
        <div className="aspect-square rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 mb-4 flex items-center justify-center overflow-hidden">
          {album.coverUrl ? (
            <img
              src={album.coverUrl}
              alt={album.title}
            ...
            <Album className="w-12 h-12 text-purple-400" />
          )}
        </div>
        <Badge variant="secondary" className="bg-purple-500/20 text-purple-300 border-0 text-xs mb-2">
        ...
        <p className="text-sm text-slate-400 truncate flex items-center gap-1">
          <User className="w-3 h-3" />
          {album.authorWallet.slice(0, 6)}...{album.authorWallet.slice(-4)}
        </p>
      </CardContent>
    </Card>
  );
...
function TrendingSection({ 
  ...
  items: Asset[];
  onLongPress: (asset: Asset, type: AssetType) => void;
}) {
    ...
    <section className="mb-12">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">Top tracks this week</p>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        {items.slice(0, 5).map((track) => (
          <TrackCard
            key={track.id}
            track={track}
            onClick={() => setLocation(`/marketplace/music/${track.id}`)}
            onLongPress={(asset) => onLongPress(asset, 'track')}
          />
        ))}
      </div>
    </section>
    ...
    <section className="mb-12">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">Complete listening experiences</p>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        {items.slice(0, 6).map((album) => (
          <AlbumCard
            key={album.id}
            album={album}
            onClick={() => setLocation(`/marketplace/music/${album.id}`)}
            onLongPress={(asset) => onLongPress(asset, 'album')}
          />
        ))}
      </div>
    </section>
...
export default function MusicHome() {
  ...
  const [contextAssetType, setContextAssetType] = useState<AssetType>('track');
  const [isContextOpen, setIsContextOpen] = useState(false);

  const { data: trendingData } = useQuery<{ items: Asset[] }>({
    queryKey: ['/api/marketplace/catalog/featured', { type: 'track' }],
    staleTime: 5 * 60 * 1000,
  });

  const { data: albumsData } = useQuery<CatalogResponse>({
    queryKey: ['/api/marketplace/catalog', { type: 'album', page: 1, limit: 6 }],
    staleTime: 5 * 60 * 1000,
  });

  const { data: catalogData, isLoading } = useQuery<CatalogResponse>({
    queryKey: ['/api/marketplace/catalog', { page, genre: genre !== 'all' ? genre : undefined, type: 'track' }],
    staleTime: 2 * 60 * 1000,
  });

  const { data: searchData, isLoading: isSearching } = useQuery<CatalogResponse>({
    queryKey: ['/api/marketplace/catalog/search', { q: searchQuery, type: 'track' }],
  ...
  const filteredItems = useMemo(() => {
    ...
    if (genre !== 'all') {
      items = items.filter((item) => item.category === genre || item.tags?.includes(genre));
    }
    ...
    return items;
  }, [displayItems, genre, sortBy]);

  const handleLongPress = useCallback((asset: Asset, type: AssetType) => {
    setContextAsset(asset);
    setContextAssetType(type);
  ...
  const handleNavigate = useCallback((assetId: string) => {
    setLocation(`/marketplace/music/${assetId}`);
  }, [setLocation]);
  ...
  return (
    <div className="min-h-screen bg-background dark:bg-slate-950">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-20 sm:pb-6">
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Music Marketplace</h1>
                <p className="text-xs text-slate-400">
                  Protocol-native digital music
                </p>
              </div>
            </div>
          </div>
          <Button
            ...
            variant="outline"
            onClick={() => setLocation('/marketplace/music/studio')}
            className="border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/10 w-full sm:w-auto min-h-[44px]"
          >
        ...
        <div className="flex flex-col gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-10 bg-card/80 dark:bg-slate-900/80 border-border/10 text-foreground placeholder:text-muted-foreground focus:border-cyan-500/50 min-h-[44px]"
            />
            {isSearching && (
        ...
        {!searchQuery && trendingData && trendingData.items.length > 0 && (
          <TrendingSection items={trendingData.items} onLongPress={handleLongPress} />
        )}

        {!searchQuery && albumsData && albumsData.items.length > 0 && (
          <AlbumsSection items={albumsData.items} onLongPress={handleLongPress} />
        )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                {filteredItems.map((track) => (
                  <TrackCard
                    ...
                    track={track}
                    onClick={() => setLocation(`/marketplace/music/${track.id}`)}
                    onLongPress={(asset) => handleLongPress(asset, 'track')}
                  />
                ))}
              </div>
              ...
              {catalogData && catalogData.hasMore && !searchQuery && (
                <div className="flex justify-center mt-8">
                  <Button
                    ...
                    className="border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/10 min-h-[44px]"
                  >
                    Load More
